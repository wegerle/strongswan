...
if [ -n "$1" ] && [ -f $CONFIG_DIR/cacerts/caCert.pem ] && [ -f $CONFIG_DIR/private/caKey.pem ]; then
  ...
  if [ -f $CONFIG_DIR/private/"$1"_Key.pem ]; then
    ...
    echo "Abort - Key for User: $1 exist"
    CLIENT_EXIST=1
  fi
  ...
  if [ $CLIENT_EXIST -eq 0  ] && [ -n $CLIENT_PASSWORD ]; then  
    ...
    #Check if STRONGSWAN_KEY_TYPE has a allowed value
    if [ "${STRONGSWAN_KEY_TYPE}" = 'RSA2048' ]; then
      ...
      echo "Abort - The value for STRONGSWAN_KEY_TYPE is wrong"
      exit 0
    fi
    ...
    if [ -f $CONFIG_DIR/private/"$1"_Key.pem ]; then
      ...
      if [ "$generated" = 'y' ]; then
        echo ""
        echo "##################################################"
        echo "#     The password for the pkcs12 (.p12) is:     #"
        echo "#                                                #"
