...
if [ -n "$2" ]; then
  ...
  if [ grep -qw "$2" /etc/ipsec.secrets ]; then
  ...
  if [ ! grep -qw "$2" /etc/ipsec.secrets  ] && [-n $CLIENT_PASSWORD]; then  
    DOMAIN_NAME=$(echo ${STRONGSWAN_HOSTNAME} | cut -d "." -f2)
    TLD=$(echo ${STRONGSWAN_HOSTNAME} | cut -d "." -f3)
    CLIENT_CN="$2"@"$DOMAIN_NAME"."$TLD"
    
    if [ "${STRONGSWAN_CLIENT_KEY_TYPE}" = 'RSA2048' ]; then
      pki --gen --type rsa --size 2048 --outform pem > $CONFIG_DIR/private/"$2"_Key.pem
      
    elif [ "${STRONGSWAN_CLIENT_KEY_TYPE}" = 'RSA3072' ]; then
      pki --gen --type rsa --size 3072 --outform pem > $CONFIG_DIR/private/"$2"_Key.pem
      
    elif [ "${STRONGSWAN_CLIENT_KEY_TYPE}" = 'RSA4096' ]; then
      pki --gen --type rsa --size 4096 --outform pem > $CONFIG_DIR/private/"$2"_Key.pem
      
    elif [ "${STRONGSWAN_CLIENT_KEY_TYPE}" = 'ECDSA256' ]; then
      pki --gen --type ecdsa --size 256 --outform pem > $CONFIG_DIR/private/"$2"_Key.pem
      
    elif [ "${STRONGSWAN_CLIENT_KEY_TYPE}" = 'ECDSA384' ]; then
      pki --gen --type ecdsa --size 384 --outform pem > $CONFIG_DIR/private/"$2"_Key.pem
      
    elif [ "${STRONGSWAN_CLIENT_KEY_TYPE}" = 'ECDSA521' ]; then
      pki --gen --type ecdsa --size 521 --outform pem > $CONFIG_DIR/private/"$2"_Key.pem
    fi
